...
The Python file consist of:
    - Algorithms for windows functions 
    - Algorithms for generating a spectogram
    - Algorithms for filters
    - Algorithms for difference Fourier transforms
"""
import numpy as np
...
def Hann2(n,M):
    while(0<=n<=M):
        return 0.5-0.5*np.cos((2*np.pi*n)/M) + 0.01
    else:
        return 0
...
def STFT(x,R,L,window = Hamming):
    N = len(x)
     #gives the highest value of r that can be evaluated
    Nr = int(np.floor(N/float(R)))
    xr = np.zeros((Nr,L)) #Matric for x_r
    #Get array with window
    w_array = [window(m,L) for m in range(L)] 
    
    #Compute sampled signal with window. This takes 16 longer than next loop
    for r in range(Nr):
        for m in range(L):
            if r*R + m <= N -1:
                xr[r,m] = x[r*R + m]*w_array[m]
    Xr = np.zeros((Nr,L),dtype = "complex") #Matric for specgrogram
    #Compute the spectrogram using numpys fft    
    for r in range(Nr):
        Xr[r] = np.fft.fft(xr[r])
    return Xr #Returns maginitude squared and the bins
  
def ISTFT(X,R,L,N,window = Hamming):
    #gives the highest value of r that can be evaluated
    Nr = int(np.floor(N/float(R)))
    xr = np.zeros((Nr,L),dtype = 'complex') #Matric for x_r
    for r in range(Nr):
        xr[r] = np.fft.ifft(X[r])
    x = np.zeros(N,dtype = 'complex')
    w = [window(n,L) for n in range(L)]
    
    for r in range(Nr):
        for n in range(R):
            n = int(n + r*R)
            indx = int(n-r*R)
            x[n] = xr[r,indx]/w[indx]
    return x
